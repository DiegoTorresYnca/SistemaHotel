  ...
  NioApp.Calendar = function () {
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
      ...
      dateClick: function(info) {
        console.log(info)
        if ($('#habitacion').val() == 0) {
          alert("Primero debe de seleccionar una habitacion.");
          return false;
        } else {
          $('#reserva-agregar #habitacion').val($('#habitacion').val());
          ...
          $('#reserva-agregar #fecha').val(info.dateStr.toString().split("-").reverse().join("/"));
      ...
      select: function(info) {
        ...
        if ($('#habitacion').val() == 0) {
          ...
          $('#reserva-agregar #check_in').val(info.startStr.toString().split("-").reverse().join("/"));
          $('#reserva-agregar #check_out').val(info.endStr.toString().split("-").reverse().join("/"));
          $('#reserva-agregar').modal('show');   
          //alert('Clicked on: ' + date.format());
          //alert('Coordinates: ' + jsEvent.pageX + ',' + jsEvent.pageY);
          //alert('Current view: ' + view.name);
      ...
      */
      events: function(fetchInfo, successCallback, failureCallback) {
        var id_habitacion = $('#habitacion').val();
        var url_reservas = "/obtener-reservas.php";

        if (id_habitacion != 0) {
          url_reservas += "?habitacion=" + id_habitacion;
        }

        console.log(id_habitacion + " // " + url_reservas);
    ...
    function llenarEventos(eventos) {
      $.each(eventos, function(idx, datos){
